...
<%--
	...
	: 클라이언트(브라우저)별로 서버에 정보를 유지하는 데 사용
	: 보안이 뛰어나다 => 보통 (사용자의 일부 정보)
	: 모든 JSP에서 사용이 가능 (공유)
	: 사용처 : 로그인시에 사용자 정보 저장 
	=> 동작
	   1. 사용자가 JSP에서 처음 접근시에 => 서버는 새로운 세션을 샹성
	   	  => 사용자(브라우저)마다 1개 생성
	...
	=> 주요 메소드
	    ...
	    ***getAttribute(String key) : 저장된 세션값 읽기
	    removeAtrribute(String key) : 특정 세션에 저장된 값 1개 삭제
	    invalidate() : 모든 세션을 해제 => logout
	    isNew() : 새로 생성된 세션 여부 확인 => 장바구니
	    ***setMaxInactiveInterval(int ms) => 기간 설정
	    	=> 1800초 (30분)
...
<body>
	...
	마지막 접속 : <%=new Date(session.getLastAccessedTime()) %>
	<h1>유효시간</h1>
	<%
